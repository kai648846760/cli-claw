...
class IflowProvider(ProviderAdapter):
    ...
    async def new_session(self, logical_session_id: str) -> str:
        provider_session_id = f"iflow:{logical_session_id}"
    ...
    async def chat_stream(self, logical_session_id: str, message: str) -> AsyncIterator[RuntimeEvent]:
        yield RuntimeEvent(
            ...
            payload={"content": f"iflow:{message}"},
        )

    async def list_models(self) -> list[str]:
        return ["iflow/default"]
